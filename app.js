...
const translations = {
  ...
  en: {
    ...
    systemKicker:'02 · HOW IT WORKS', systemTitle:'One system.<br>Five stages.', systemText:'Select each stage to see how AEDES-X components work together.', stage1:'Power & Input', stage1p:'Solar panel, main switch, mode button and LDR sensor.', stage2:'System Brain', stage2p:'The ESP32 processes inputs and coordinates operation.', stage3:'Attraction & Airflow', stage3p:'UV LEDs and the fan provide attraction and suction.', stage4:'Capture', stage4p:'A fine-mesh basket securely retains mosquitoes.', stage5:'CO₂ Generator', stage5p:'Yeast, sugar and warm water produce an attraction signal.',
...
function applyLanguage(language) {
  ...
  if (activeStage) $('#stageLabel').textContent = stageLabels[language][Number(activeStage.dataset.stage)];
...
const stageLabels = {
  ...
  en: ['POWER & INPUT', 'SYSTEM BRAIN', 'ATTRACTION & AIRFLOW', 'CAPTURE', 'CO₂ GENERATOR']
};
$$('.stage').forEach((button, index) => {
  button.addEventListener('click', () => {
    ...
    $('#stageLabel').textContent = stageLabels[currentLanguage][index];
    const image = $('.exploded-wrap img');
    image.style.transform = `translateY(${(2 - index) * 7}px) scale(${1 + index * 0.01})`;
    image.style.filter = `drop-shadow(0 30px 60px rgba(0,0,0,.5)) hue-rotate(${index * 8}deg)`;
  });
});
